...
### Setapp
[Setapp](https://setapp.com/) provides lots of high-quality Mac (and iOS) apps under a subscription service. It uses `Setapp.app` to manage apps installed in `~/Setapp/`, so I use this Hazel rule to generate a list of installed Setapp apps whenever a new app is added. The list output is `~/.dotfiles/install_lists/setapp-install.txt`, alongside the repo's other install lists. The rule simply runs an embedded form of `~/.dotfiles/utility/setapp-install.sh`, so you can do this without Hazel as well — but note the embedded copy is exactly that, a copy: `Setapp.hazelrules` is a binary export, so when the script changes, the rule has to be updated inside Hazel by hand and re-exported here.
